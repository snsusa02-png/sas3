...
//        ]);
    ...
    public
    function rep72(Request $request)
    {
        //Данные по покупкам/продажам по перевозкам
        $report_id = 72;
    ...
    , if(mro.sale_dir=-1, ri.name, null) as buy_itmname
    , if(mro.sale_dir=-1, mro.itm_sum, 0) as buy_sum
    , if(mro.sale_dir=-1, mro.itm_qty, 0) as buy_qty
    , if(mro.sale_dir=-1, if(ri.unittypeid=8, mro.itm_qty, round(mro.itm_qty / riu.k2ref_unit,3)), 0) as buy_qty_m3
    , org.name as org_name
    , mro.org_placename
    , if(mro.sale_dir=1, ri.name, null) as sale_itmname
    , if(mro.sale_dir=1, mro.itm_sum, 0) as sale_sum
    , if(mro.sale_dir=1, mro.itm_qty, 0) as sale_qty
    , if(mro.sale_dir=1, if(ri.unittypeid=8, mro.itm_qty, round(mro.itm_qty / riu.k2ref_unit,3)), 0) as sale_qty_m3
    , mro.raid_qty as raid_qty
	from mr_opers mro
	join mchn_raids as mr on mr.id=mro.mr_id
    join driver_works dw on dw.id=mr.dw_id
    join opertypes ot on ot.id=ifNull(dw.opertypeid, 1)
    ...
    left join ri_units riu on riu.refitmid=ri.id and riu.unittypeid=8

    where {$sc1}
    order by mr.wrkdate, os.lname, dw.staffid, mr.id, mro.sale_dir";
        ...
        if ($export2xls == "1") {
            ...
            $response = Excel::download(new rep72Export($recs, $data), $xls_fileName, \Maatwebsite\Excel\Excel::XLSX);
