...
//            dd($aCnds, $aGrps, array_diff($aCnds, $aGrps), array_diff($aGrps, $aCnds));
            ...
            if (isset($begdate2) and isset($enddate2)) {
                ...
                if (isTblInGrps('p_l', $grps))
                    $recs2 = $recs2->leftjoin('org_places as p_l', 'p_l.id', 'fd.load_placeid');
                ...
                $recs2 = $recs2->selectRaw('2 as dataset, sum(fd.mchnwrkhrs*1) as itmqty
                , sum(fd.unload_sum) itmsum
                , sum(fd.raid_qty) raid_qty
                , count(distinct fd.id) as docqty');
            ...
            $recs = mchn_raid::from('mchn_raids as fd')
                //->join('machines as m', 'm.id', 'fd.machineid')
                ->wherebetween('fd.wrkdate', [$begdate1, $enddate1])
                ->whereIn('fd.active', [1])
                ->whereraw($sc);
            ...
            if (isTblInGrps('p_l', $grps))
                $recs = $recs->leftjoin('org_places as p_l', 'p_l.id', 'fd.load_placeid');
...
//            $recs = $recs->selectRaw('1 as dataset, sum(di.qty*ft.forsale) as itmqty, sum(di.qty*di.price*ft.forsale) itmsum
//                , count(distinct fd.id) as docqty');
            $recs = $recs->selectRaw('1 as dataset, sum(fd.mchnwrkhrs*1) as itmqty
                , sum(fd.unload_sum) itmsum
                , sum(fd.raid_qty) raid_qty
                , count(distinct fd.id) as docqty');
