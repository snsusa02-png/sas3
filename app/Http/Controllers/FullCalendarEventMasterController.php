...
class FullCalendarEventMasterController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data->s_users = User::from('users as u')
            ->where('u.active', 1)
            ->whereRaw("(
            exists( select 1 from events as e where e.inituserid=u.id)
        ...
        return view('events/eventcalendar', compact('data'));
    }
    ...
    public function get()
    {
        ...
        if (1 === 1) {
            $data = event::from('events as e')
                ->leftjoin('users as iu', 'iu.id', 'e.inituserid')
                ->leftjoin('users as ru', 'ru.id', 'e.updated_by')
                ->where('e.active', 1)
                ->whereDate('e.begdt', '>=', $start)
                ->whereDate('e.enddt', '<=', $end)
                ->whereraw($sc)
                ->get([
                    'e.id', 'e.title'
                    , 'e.begdt as start', 'e.enddt as end'
                    , 'e.color'
                    , 'e.inituserid'
                    , 'e.descript'
                    , 'e.event_place'
    ...
    public function update(Request $request)
    {
        ...
        if (isset($id)) {
            ...
            $updateArr = ['title' => $request->title
                , 'begdt' => $request->start
                , 'enddt' => ($request->end ?? $request->start)
                , 'event_place' => $request->event_place
                , 'color' => $request->color
                , 'inituserid' => $request->userid ?? $userid
                , 'descript' => $request->descript
