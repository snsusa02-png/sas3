        ...
        if (1 == 1) {
            $sells = mr_oper::from('mr_opers as mro')
                ...
                ->select('mr.wrkdate as operdate', db::raw('2 as sumtypeid')
                    ...
                    , db::raw("sum((select sum(pd.paydir*pd.paysum)	from obj_links ol  join paydocs as pd on pd.id=ol.lnkobjid	where ol.sysobjid=1107 and ol.lnksysobjid=520 and ol.objid=mro.id )) as paid_sum")
                )
            ...
            $wrh_sells = wrhdoc::from('wrhdocs as d')
                ...
                ->select('d.docdate as operdate', db::raw('2 as sumtypeid')
                    ...
                    , db::raw("concat(os.lname, ' ', os.fname) as disp_name")
                    , db::raw("sum((select sum(pd.paydir*pd.paysum)	from obj_links ol join paydocs as pd on pd.id=ol.lnkobjid	where ol.sysobjid=204 and ol.lnksysobjid=520 and ol.objid=d.id )) as paid_sum")
                )
