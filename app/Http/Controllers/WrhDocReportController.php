...
//        //dd($data->fuelcards);
    ...
    public function rep74(Request $request)
    {
        //
        $report_id = 74;
        ...
        $param_names = [
            's_pageitmcnt' => 20
            , 's_ownorgid' => '' //Auth::user()->curorgid
            , 's_saleorgid' => '' //Auth::user()->curorgid
            , 's_period_type' => 1
            , 's_begdate' => $pd //$fdomc
            , 's_enddate' => $pd //$ldomc
            , 's_month' => $month
        ...
        foreach ($search_params as $item => $val) {
            if (isset($val) and strlen($val) > 0) {
                ...
                if ($item == 's_ownorgid') {
                    $sc = $sc . " and d.orgid = '{$val}'";
                }
                elseif ($item == 's_saleorgid') {
                    $sc = $sc . " and d.saleorgid = '{$val}'";
	...
	, sum(i.qty) as itmqty, sum(sum) as itmsum
    from wrhdoclst as i
    join refitems as ri on ri.id=i.refitmid
	join wrhdocs d on d.id=i.docid
