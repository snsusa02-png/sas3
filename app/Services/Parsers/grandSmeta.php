...
//                    dd($chapter['Caption'], $estdoc_section, $estdoc_section->id);
                    ...
                    $ii = 0;
                    $jj = $chapter->Header->count() - 1; //будем формировать задом наперёд
                    $h = array();
                    $subsect_ordr = 0;
                    foreach ($chapter->Header as $header) {

                        $subsection = new estdoc_section([
                            'estdocid' => $doc->id,
                            'parid' => $sectionid,
                            'sysid' => $chapter['SysID'],
                            'name' => $header['Caption']->__toString(),
                            'ordr' => ++$subsect_ordr,
                        ]);
                        $subsection->save();
...
//                        dd($h);
                    ...
                    $aPositionHeader = [];
                    $aPositionSectId = [];  //тоже но estdoc_sections.id для Header
                    $ii = 0;
                    ...
                    foreach ($chapter->Position as $position) {
                        ...
                        //только если позиция имеет название и нет опции Inactive

                        $may_add = true;
                        if (isset($position['Options'])) {
                            //не добавляем позиции отмеченные как Неактивные
                            $may_add = !($position['Options'] == 'Inactive');
                        }

                        if (isset($position['Caption']) and $may_add) {
                            ...
                            $itmtypeid = 1;  //временно?
                            //dd($aPositionHeader[$position_npp], $aHdr);

                            //попробуем найти номенклатуру по коду ресурса во внешней системе (6=GrandSmeta)
                            $refitmid = objextid::objid_by_extsysid_extid(6, 105, $position['Code']);

                            //попробуем найти id типа ЕИ по названию Единицы измерения
                            $unittypeid = unittype::idByName($position['Units']);

                            $docitem = new estdocItem([
                                'estdocid' => $doc->id,
                                'parid' => $parid,
                                'sectionid' => $aPositionSectId[$position_npp] ?? $sectionid,
                            ...
                            if (1 == 1 and $docitem->itmtypeid == 2 and isset($parid)) {
                                ...
                                $itm_resource = new estdocitm_resource([
                                    ...
                                    'ressum' => $ressum,
                                    'ordr' => 1,    //выносим вперед
                                    'grp' => 1,    //признак необходимости закупки
                                ]);
                                $itm_resource->save();

                                //увеличим сумму материалов/ или механизмов для соотв. позиции
                                $paritm = estdocItem::find($parid);
                                //скорректируем итоги родительской записи
                                if ($resource_kind == 'Mch')
                                    $paritm->exp_m_sum += $ressum;
                                else
