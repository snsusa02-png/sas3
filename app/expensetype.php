...
class expensetype extends Model
{
    ...
    static public $prefix = 'expensetypes';
        ...
        foreach ($params as $key => $val) {
            ...
            if (isset($val) and $val !== '') {
                ...
                if (array_search($key, $used_params) == 0) {
                    ...
                    if ($key == 'active') {
                        $sc .= " and et.active={$val}";

                    } elseif ($key == 'active_or_current') {
                        $sc .= " and (et.active=1 or et.id={$val})";
                    ...
                    } elseif ($key == 'in_obj_expenses') {
                        $sc .= " and " . (($val == 0) ? "not" : "")
                            . " exists (select 1 from obj_expenses as oe where oe.expensetypeid=et.id)";
    ...
    {
        //2021-02-25 SNS. универсальный конструктор массива с id, name expensetypes
        ...
        if (isset($params) and is_countable($params) and count($params) > 0) {
            ...
            $lst = self::from('expensetypes as et')
                ->whereRaw($sc)
                ->select('id', 'name')
                ->orderBy('et.name', 'desc')
    ...
    {
        //2021-04-30 SNS. универсальный конструктор коллекции из записей expensetypes
        // params - массив, содержащий пару "имя параметра"=>"значение параметра"
        ...
        if (isset($s_params) and is_countable($s_params) and count($s_params) > 0) {
            ...
            $sorts = $sorts ?? [['et.name', 'asc']];

            $recs = self::from('expensetypes as ot')
                ->whereRaw($sc)
                ->select($fields);
