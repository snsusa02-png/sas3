...
@section('content')
    ...
    @else
        @if (!isset( $rec))
            <?php
            redirect()->route('payrolltypes.index');
            header("Location:" . route('payrolltypes.index'));
            die();
            ?>
        @else
            <?php
            ...
            $thisTitle = "Схема расчета заработанной платы сотрудников";
                        ...
                        <div class="card mt-3">
                            ...
                            <div class="card-body">
                                ...
                                <form name="forEdit" id="forEdit" method="post"
                                      action="{{ route('payrolltypes.update', $rec->id) }}">
                                    @method('PUT')
                                    @csrf
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control rounded-0" name="descript" id="descript"
                                                  rows="5">{{ old('descript',$rec->descript) }}</textarea>
                                    </div>
                                    <div class="row">
...
'title'=>'Доступно для использования']) !!}
                                    ...
                                    @if ($usrrights['delete'])
                                        <button type="submit"
                                                ...
                                                formaction="{{ route('payrolltypes.delete', $rec->id)}}"
                                                formmethod="post"
                                                title="Удалить"
                    ...
                    @if($rec->id <> -1)
                        <div class="col-md-5">
                            @include('payrolltypes._rate_sets')
                            @include('payrolltypes._ref_staff')
                        </div>
                    @endif
