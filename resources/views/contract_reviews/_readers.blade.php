@if( 1==1 and isset($rec) and ($rec->id!=-1) and isset($rec->contrrev_users))
	<div class="card mt-3 d-none d-sm-block">
		<div class="card-header">
			<span data-toggle="collapse" data-target="#contrrev_users"><i class="fa fa-users text-success"
																   aria-hidden="true"></i> Участники согласования</span>

			<div class="float-right">
				@if (count($rec->contrrev_users)>0)
					<button data-toggle="collapse" data-target="#contrrev_users"
							class="btn btn-light btn-sm "><i class="fa fa-eye-slash" aria-hidden="true"></i>
						<span class="badge badge-info">{{count($rec->contrrev_users)}}</span>
					</button>
				@endif

				@if( $usrrights['save'])
					<a href="{{ route('contrrev_users.create',['contrrevid'=>$rec->id])}}"
		...
		@if (count($rec->contrrev_users)>0)
			<div class="card-body collapse" id="contrrev_users">
				<table class="table-condensed table-striped small" style="width: 100%;">
					<thead>
					<tr>
						<td>#</td>
						<td colspan="2">Согласователь</td>
						<td/>
					</tr>
					</thead>
					<tbody>
                    ...
                    ?>
					@foreach($rec->contrrev_users as $itm)
                        <?php
						...
						<tr class="align-top">
							...
							<td class="text-right">
								<a href="{{ route('contrrev_users.edit',$itm->id)}}?returl={{Route::currentRouteName()}}"
			...
			<div class="card-footer">
				<div class="small text-right"> всего участников: {{count($rec->contrrev_users)}},
