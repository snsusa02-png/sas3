...
@section('content')
	<script src="{{ asset('js/handleListOrgStaff.js') }}"></script>
	@guest
        <?php
	...
	@else
		...
		<form name="forIndex" id="forIndex" method="post" action="{{ route('orgstaffs.list') }}">
			@csrf
					...
					<div class="col-md-12">
						...
						<table class="table table-condensed tbl_list" style="background-color: white;">
							...
							<tr style="text-align: center;">
								...
								<th>
									<div class="input-group">
										<input type="text" class="form-control c" name="s_postname"
											   value="{{$s_postname ?? ''}}"/>
									</div>
								</th>
								<th>
									<div class="input-group-btn">
										<button type="submit" class="btn btn-sm btn-outline-secondary"
												formaction="{{ route('orgstaffs.list') }}"
