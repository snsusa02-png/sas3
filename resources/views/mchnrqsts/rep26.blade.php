...
@section('content')
    ...
    <?php
    $thisTitle = "Отчет о работе спецтехники и механизмов по заявкам арендатора за период";
                    ...
                    <div class="card-body">
                        ...
                        <form name="forRep01" id="forRep01" method="post"
                            ...
                            <div class="row">
                                ...
                                @if( 1==0 and $usrrights['set_paytype'] )
                                    <div class="form-group col-md-3">
...
{{--                                    {{ ($rec->rqsttypeid==2) ? $rec->tgt_addr: $rec->src_addr.' - '. $rec->tgt_addr}}--}}
                                    {{ ($rec->rqsttypeid==2) ? $rec->name: $rec->src_addr.' - '. $rec->tgt_addr}}
                                    <a href="{{route('mchnrqsts.edit',$rec->id)}}" target="_blank" class="d-print-none">...</a>
                                    <div class="ml-2 small float-right">
                                        {{$fctwrkperiod}}
                                    </div>
                                </td>
                                <td class="small">{{$rec->mchnname??$rec->asgnmachinename}}</td>
