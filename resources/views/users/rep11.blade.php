...
@section('content')
    ...
    <?php
    $thisTitle = "Статистика операций пользователей в ИС по часам суток";
    $thisSysObjId = 855;    //reports
    $thisObjId = 11;
    $action_url = route('reports.rep11');
    ...
    <style>
        ...
        .maxmark {
            fill: #c21918; /* changes the background */
        }
            ...
            @else
                ...
                <div class="page p-2 ">
                    ...
                    <table class="rep-data mt-3" style="background-color: snow; font-size:16px; width:960px"
                        ...
                        $maxCnt = 0;
                        foreach ($recs as $rec) {
                            $maxCnt = ($maxCnt < $rec->cnt) ? $rec->cnt : $maxCnt;
                        }
                        $k = ($maxCnt == 0) ? 0 : ($maxWidth / $maxCnt);
                        ?>
                        @foreach($recs as $rec)
                            <?php
                            $g_class = ($rec->cnt == $maxCnt) ? 'maxmark' : 'done';
                            ?>
                            <tr class="text-left">
                                <td class="text-center">{{$rec->mark}}</td>
                                <td class="text-center">{{number_format($rec->cnt,0)}}</td>
                                <td>
                                    <svg class="chart" width="{{$maxWidth}}" height="21" aria-labelledby="title desc"
                                         role="img">

                                        @if($rec->cnt>0)
                                            <g class="bar {{$g_class}}">
                                                <rect width="{{$k*$rec->cnt}}" height="19"></rect>
                                                <title id="title1">Количество операций</title>
                                                <text x="5" y="9.5" dy=".35em">{{$rec->cnt}}</text>
                                            </g>
                                        @endif
                                    </svg>
                                </td>
