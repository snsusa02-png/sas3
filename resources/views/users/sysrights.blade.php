...
@section('content')
    ...
    if (!$usrrights['edtrights']){
        ...
        header("Location:" . route('users.edit', $userid));
        die();
    }

    $retURL = \Request::get('returl') ?? $data->retURL ?? (route('users.index') . "?page=" . session('users' . '_pageno') . '#' . $userid);
    ?>
    <script>
        function to_top0() {
            $('html, body').animate({
    ...
    <div class="container card my-2 col-md-6">
        <h4 class="pt-2">Редактирование прав
            ...
            <div class="text-center">Пользователь: {{$user->name}}</div>
            @if(isset($data->limsysobjid))
                <div class="text-center small">Область: "{{$data->limobj_name??''}}" ({{$data->limsysobj_name??''}})</div>
            @endif

        </h4>


        <form action="{{ route('users.updatesysrights', ['id'=>$data->userid, 'limsysobjid'=>$data->limsysobjid??0, 'limobjid'=>$data->limobjid??0]) }}">
    ...
    <div class="card">
        ...
        <div id="collapse{{$itm->objid}}" class="collapse" aria-labelledby="heading{{$itm->objid}}"
            ...
            <div class="card-body">
                ...
                if (isset($usrsysrights[$itm->id])) {
                    $checked = "checked";
                    $userset = $usrsysrights[$itm->id][0]->userset;
                    $created_at = $usrsysrights[$itm->id][0]->created_at;
                    $whowhn = "(" . $created_at . ", " . $userset . ")";
                } else {
                    $checked = "";
                    $whowhn = "";
                }
                ...
                <div class="row">
                    <div class="offset-md-1 col-md-11">
                        ...
                        @if($checked and $usrsysrights[$itm->id][0]->func_org_area==1)
                            <a href="{{route('usrsysright_orgs.index',$usrsysrights[$itm->id][0]->id)}}"
                               title="область применения права по организацям" style="float:right">область</a>
                        @endif
                    </div>
                </div>
                @endforeach
    ...
    <div class="row" id="actions0">
        <div class="col-md-12 text-center my-3">
            ...
            <a class="btn btn-close btn-info" href="{{ route('users.edit', $userid) }}">
