...
@section('content')
    <?php
    $report_id = 34;
    $thisTitle = $data->report->name ?? "Товарный запас по МОЛ";
    ...
    <div class="container">
                            ...
                            @if(1==1)
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-3">
                                        <label for="s_begdate">МОЛ:</label>
                                        {!! Form::select('s_mol_staffid', $data->mols??[], $search_params['s_mol_staffid'] ?? '',
                                             [
        ...
        @if (isset($recs))
            ...
            @else
                <?php
                $s_mol_staffid = $search_params['s_mol_staffid'] ?? '';
                ...
                <div class="page p-2 " style="background-color: white">
                    ...
                    <table class="table table-sm table-striped rep-data mt-3"
                        ...
                        <tr>
                            <td class="small">#</td>
                            <td>МОЛ, Владелец, Наименование материала/товара</td>
                        ...
                        @foreach($recs as $itm)
                            ...
                            @if($itm->staffid<>$curStaffID)
                                <tr style="background-color: #fdffd1">
                                    ...
                                    <td colspan="6" class="font-italic">
                                        склад: <b>{{$itm->mol_name}}</b>
                                    </td>
                                </tr>
                                <?php
                                $curStaffID = $itm->staffid;
                                $curWrhName = $itm->mol_name;
                                $wrhSum = 0;
                                $curBoxID = -1;
                                $curOwnOrgCnt = 0;
                                ?>
                            ...
                            @if( $itm->itmtypeid<>$curItmTypeID)
                                ...
                                $curItmTypeName = '"' . $itm->itmtype_name . '"';
                                $curItmTypeCnt++;
                                $ItmTypeSum = 0;
                                ?>
                            @endif
