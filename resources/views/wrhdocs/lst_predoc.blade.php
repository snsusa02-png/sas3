            ...
            <table class="table-striped small"
                ...
                @foreach($items as $itm)
                    ...
                    <tr class="align-top">
                        ...
                        <td class="text-center " nowrap>
                            <span class="{{$qty_class}}">{{number_format($itm->qty,3)}}</span>
                            из {{number_format($itm->prelst->qty,3)}}
                        </td>
                        @if ($showPrice)
                            <td class="text-right">
                                {{$itm->price}}
                ...
                <tr>
                    <td colspan="{{$tot_span_colno}}" class="text-right">
                        @if($usrrights['make_diffdoc']??false and $qty_diff_cnt>0)
                            <a href="{{route('wrhdocs.make_diffdoc', $rec->id)}}" class="btn btn-warning mr-3">
                                Зарегистрировать потери
                            </a>
                        @endif
                        Итого:
                    </td>
