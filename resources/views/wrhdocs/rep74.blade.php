                        ...
                        <form name="forRep01" id="forRep01" method="post"
                            ...
                            @if(1==1)
                                <div class="row">
                                    ...
                                    @if(1==1)
                                        <div class="form-group col-md-3">
                                            <label for="s_ownorgid" class="">Продавец:</label>
                                            {!! Form::select('s_saleorgid', $data->saleorgs??[], $search_params['s_saleorgid'],
                                                            [
                                                            'class' => 'form-control',
                                                            'placeholder' => '-все-',
                                                            ])
                ...
                <div class="page p-2 container-fluid">
                    ...
                    <table class="table table-sm table-striped rep-data mt-3"
                        ...
                        <tr class="text-left small" valign="top">
                            ...
                            <td class="text-left">Покупатель</td>
                        ...
                        <?php
                        $npp = 0;
                        $totSum = $saleSum = $orgSum = 0;
                        $cur_saleorgid = $cur_orgid = -1;
                        $cur_saleorgname = $cur_orgname = '';
                        ?>
                        @foreach($recs as $rec)

                            @if($rec->saleorgid <> $cur_saleorgid)
                                @if($cur_orgid <> -1)
                                    <tr>
                                ...
                                @if($cur_saleorgid <> -1)
                                    <tr>
                                        <td colspan="5" class="text-right text-success">Всего по "{{$cur_saleorgname}}":</td>
                                ...
                                @endif
                                <tr>
                                    <td colspan="6" class="font-weight-bold text-success">{{$rec->saleorgname}}</td>
                                </tr>
                                <?php
                                $cur_saleorgid = $rec->saleorgid;
                                $cur_saleorgname = $rec->saleorgname;
                                $cur_orgid = -1;
                                $cur_orgname = '';
                                ?>
                            @endif
                            @if($rec->orgid <> $cur_orgid)
                                ...
                                <tr>
                                    <td></td>
                                    <td colspan="5" class="font-weight-bold">{{$rec->orgname}}</td>
                                </tr>
                                <?php
                                $cur_orgid = $rec->orgid;
                                $cur_orgname = $rec->orgname;
                                ?>
                            @endif
                            <?php
                            $td_class = '';
                            ...
                            <tr class="text-left collapse show date_{{$tr_date??''}} multi-collapse">
                                ...
                                <td></td>
                                <td class="text-left ">
{{--                                    <a href="{{route('refitems.edit',$rec->refitmid)}}" target="_blank" class="text-decoration-none">--}}
                                    {{$rec->itmname}}
{{--                                    </a>--}}
                                </td>
