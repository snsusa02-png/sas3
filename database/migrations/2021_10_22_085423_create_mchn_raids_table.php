...
//            $table->decimal('wrkhrs',4,1)->nullable()->comment('кол-во рабочих часов');
	    ...
	    $table->decimal('driver_sum', 12,2)->nullable()->comment('Общая стоимость для заказчика-посредника от организации-перевозчика car_orgid');
            ...
            $table->biginteger('load_placeid')->unsigned()->nullable()->comment('ID места погрузки');
            $table->string('load_placename',60)->nullable()->comment('Название(адрес) места погрузки');
	
            $table->biginteger('unload_placeid')->unsigned()->nullable()->comment('ID места выгрузки');
            ...
            $table->decimal('unload_qty', 8,2)->nullable()->comment('кол-во загруженного (в единицах qty_unit)');
...
//		$table->foreign('buildopertypeid')->references('id')->on('buildopertypes');
            ...
            $table->biginteger('ownorgid')->unsigned()->nullable()->comment('Организация-исполнитель (владелец)');
		$table->foreign('ownorgid')->references('id')->on('orgs');

            $table->biginteger('orgid')->unsigned()->nullable()->comment('Организация-подрядчик');
...
//		$table->foreign('bdgtitmsumid')->references('id')->on('budget_itmsums');
            ...
            $table->biginteger('disp_userid')->unsigned()->nullable()->comment('Диспетчер (пользователь ИС)');
		$table->foreign('disp_userid')->references('id')->on('users');
